...
class WeatheryManager {
    func getCurrentWeather(latitude: CLLocationDegrees, longitude: CLLocationDegrees) async throws -> ResponseData {
        guard let url = URL(string: "https://api.openweathermap.org/data/2.5/forecast/hourly?lat=\(latitude)&lon=\(longitude)&appid=\(Secrets.apiKey)&units=metric") else {
...
//    private let apiKey = "33622cd9552637c8e26ce9bf1d92bc7d"
//    private let baseUrl = "https://api.openweathermap.org/data/2.5"
//    
//    func getWeather(for city: String) -> AnyPublisher<WeatherResponse, Error> {
//        let urlString = "\(baseUrl)/weather?q=\(city)&appid=\(apiKey)&units=metric&lang=de"
...
//    }
//    
//    func getForecast(for city: String) -> AnyPublisher<ForecastResponse, Error> {
//        let urlString = "\(baseUrl)/forecast?q=\(city)&appid=\(apiKey)&units=metric&lang=de"
//        guard let url = URL(string: urlString) else {
//            return Fail(error: URLError(.badURL)).eraseToAnyPublisher()
//        }
//        return URLSession.shared.dataTaskPublisher(for: url)
...
//func getCurrentWeather(latitude: CLLocationDegrees, longitude: CLLocationDegrees) async throws -> ResponseData {
//        guard let url = URL(string: "https://api.openweathermap.org/data/3.0/onecall?lat=\(latitude)&lon=\(longitude)&exclude=minutely,alerts&appid=\(Secrets.apiKey)&units=metric") else {
//            throw NSError(domain: "WeatheryManager", code: 1, userInfo: [NSLocalizedDescriptionKey: "Missing URL"])
//        }
//        
//        let urlRequest = URLRequest(url: url)
